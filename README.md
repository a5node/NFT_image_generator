# Welcome to A5 Fork of HashLips! 👄

Fork purpose: Added rule for like "don't combine trait x with trait y from another layer". In config layerConfigurations sections add

```js
omitLayersCombinations: [
      {combination: "Eyeball.Red&Iris.Large"},
      {combination: "Eye color.Red&Iris.Small"},
      {combination: "Eye color.Red&Iris.Large&Top lid.Low"},
    ],
```

In this example value 'Red' from 'Eyeball' layer will not be combined with 'Large' from 'Iris' layer etc. Traits combination concidered to be not valid if contains ALL values from 'omitLayersCombinations.combination'. So for "Eye color.Red&Iris.Large&Top lid.Low" combination containing 'Eye color.Red' and 'Iris.Large' considered to be OK, and only if it has 'Top lid.Low' as well - then not.
